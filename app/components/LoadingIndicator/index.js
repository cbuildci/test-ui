...
function ImportLoadingIndicator({
    error,
    timedOut,
    retry,
    pastDelay,
    ...
    if (error) {
        return (
            <div>
                <div>Error! <button className="btn" onClick={retry}>Retry</button></div>
        ...
        return (
            <div>
                Taking a long time... <button className="btn" onClick={retry}>Retry</button>
            </div>
        );
    }
    else if (pastDelay) {
...
LoadingIndicator.ForImport = ImportLoadingIndicator;
